...
#           receiver.
#
# Code   : Radio and ALSA sound class.
...
class Radio:
	...
	soundPaused = False # Pause MPD from playing
	...
	# ALSA Options
	volumeid = 2  # The number for the ALSA master volume.
	switchid = 1  # The number for the ALSA mute switch ID.
	...
	def setVolume(self,volume):
		if self.soundMuted: 
			...
			self.volume = volume
			self.execCommand("amixer cset numid=" + str(self.volumeid) + " " + str(self.volume) + "%" )
	...
	def paused(self):
		status = self.execMpcCommand("status | grep \"\[paused]\" ")
		if ( "[paused]" in status ):
			self.soundPaused = True
		else:
			self.soundPaused = False
		return self.soundPaused
	...
	def getDisplayModeString(self):
		sMode = ["MODE_TIME", "MODE_SEARCH", "MODE_SOURCE",
			 "MODE_OPTIONS", "MODE_RSS", "MODE_IP", "MODE_AIRPLAY"] 
