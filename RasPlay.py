...
class MyDaemon(Daemon):
	...
	def run(self):
		...
		count = 0 
		while True:
			get_switch_states(lcd,radio)
			radio.setSwitch(0)

			if checkAirplay():
				log.message("Airplay Mode On", log.INFO)
				radio.pause()
				display_mode = radio.MODE_AIRPLAY
			else:
				if display_mode == radio.MODE_AIRPLAY:
					log.message("Airplay Mode Off", log.INFO)
					radio.unpause()
			...
			elif display_mode == radio.MODE_AIRPLAY:
				todaysdate = strftime("%H:%M %d/%m/%Y")
				lcd.line1(todaysdate)
				lcd.line2("Airplay mode")
...
def interrupt():
	global lcd
	global radio
	interrupt = get_switch_states(lcd,radio)
	return interrupt
...
def checkAirplay():
	airplay_status = radio.execCommand("tail -1 /var/log/shairport.state")

	if ( "playing on" in airplay_status ):
		airplay = True
		airplay_client = airplay_status[18:]
	else:
		airplay = False
		airplay_client = False

	return airplay

# Check switch states
def get_switch_states(lcd,radio):
